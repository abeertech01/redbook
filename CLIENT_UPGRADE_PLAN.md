...
- [x] Real-time chat: new chat, send/receive messages live via Socket.IO (`pages/Messages.tsx`, `Inbox.tsx`, `ChatParticipator.tsx`, `SocketProvider.tsx`, `useSocketEvents.ts`) — **found and fixed, pre-existing bug (confirmed present on `main` too, not a regression)**. Root cause: `SocketProvider` created the Socket.IO connection inside `useMemo`, which React explicitly does not guarantee runs only once. Under React 19 + StrictMode's dev-mode double-invoke, this created two real connections per page load; only one had listeners attached, and the server could end up registering the orphaned, unlistened one as "current" for that user, so live events were dispatched into the void. Verified server-side delivery was never the issue via a standalone Node script (`socket.io-client`, no React) that connected two raw sockets and confirmed instant, correct delivery both ways. Fixed by moving connection creation into `useEffect` with a real `disconnect()` cleanup (the pattern StrictMode expects), and hardened `useSocketEvents` (a related but secondary bug: it was re-subscribing its listeners on every render because callers pass a fresh handlers object each time) to use a ref so it only subscribes once per socket. Also fixed along the way: `Messages.tsx` wasn't navigating into newly-created chats, and `Inbox.tsx` showed the wrong name above every message (compared the chat participant's id to your own id, which is always false, instead of checking the actual message author). Verified live: both sides now see new chats and messages appear immediately, no reload needed
